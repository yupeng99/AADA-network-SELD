...
class SolverBasic(SolverGeneric):
    def __init__(self, config, tensorboard_writer=None, model_checkpoint=None):
        ...
        if config.model =="baseline":
            config.input_shape = (7, 500 , 64)
            config.output_shape = (50, 117)
            summary(self.predictor, input_size=tuple([1, *(7, 64, 500)]),
                    col_names=['input_size', 'output_size', 'kernel_size', 'num_params'],
                    row_settings=['var_names'], verbose=1)
        elif config.model =="crnn10":
            config.input_shape = (7, 256, 256)
            config.output_shape = (3, 12, 256)
            self.config.input_shape = (7, 256, 256)
            print(f'Input predictor = {self.config.input_shape}')
            summary(self.predictor, input_size=tuple([1, *self.config.input_shape]),
                    col_names=['input_size', 'output_size', 'kernel_size', 'num_params'],
                    row_settings=['var_names'], verbose=1)
        elif config.model == "resnet-conformer":
            config.input_shape = (7, 500, 64)
            summary(self.predictor, input_size=tuple([1, *(7, 64, 500)]),
                    col_names=['input_size', 'output_size', 'kernel_size', 'num_params'],
                    row_settings=['var_names'], verbose=1)
    ...
    def build_predictor(self):
        if self.config.model == 'crnn10':
            ...
            predictor = SeldModel(self.config)
        elif self.config.model == 'resnet-conformer':
            predictor = ResnetConformer(in_feat_shape=(self.config.input_shape))
    ...
    def curriculum_scheduler_step(self, step: int, val_loss=None, seld_metric=0):
        ...
        if self.curriculum_scheduler == 'fixed':  # No update, augmentaiton always active
            ...
            #     update_every = int((self.config.num_iters *2) /20 )
            # else:
            update_every = self.config.logging_interval*5
    ...
    #                                                                                          self.data_gen['y'],
    #                                                                                          alpha=self.config.mixup_alpha,  # 可以改成 参数化
    ...
    def forward(self, x, y):
        # mixup
        if self.config.use_mixup:
            x, y_a, y_b, lam = mixup_data(x, y, alpha=self.config.mixup_alpha,  # 可以改成 参数化
                                               use_cuda=self.device == 'cuda')
        else:
            lam = 1.
        y_hat = self.predictor(x)
        if self.config.use_mixup:
            return x,y_hat,y_a,y_b,lam
        else :
            return y_hat,lam
    ...
    def backward_predictor(self,criterion,y_hat=None,y=None,y_a=None,y_b=None,lam=None,):
        """Calculate GAN and reconstruction loss for the generator"""


        if self.config.use_mixup:
            loss_func = mixup_criterion(y_a, y_b, lam)
            loss_G_rec = loss_func(criterion, y_hat)
            loss_G_spec = 0.0
        else:
            loss_G_rec = criterion(y_hat, y)
            if self.config.w_rec_spec > 0.0:
                loss_G_spec = self.loss_fns['G_rec_spec'](y_hat, y)
            else:
                loss_G_spec = 0.0
        # Total weighted loss
        # self.loss_values['G_rec'] = loss_G_rec
    ...
    def train_step(self):
        ...
        self.predictor.train()  # predictor 是model
        self.forward()
        # Update Predictor
