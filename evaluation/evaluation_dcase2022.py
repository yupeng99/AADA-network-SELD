...
def write_output_format_file(config,_output_format_file, _output_format_dict, use_cartesian=True, ignore_src_id=False):
    """
    ...
    for _frame_ind in _output_format_dict.keys():
        for _value in _output_format_dict[_frame_ind]:
            if use_cartesian:
                if '2023' in config['dataset_root_valid']:
                    _fid.write(
                        '{},{},{},{},{},{},{}\n'.format(int(_frame_ind), int(_value[0]), 0, float(_value[1]),
                                                        float(_value[2]),
                                                        float(_value[3]), 0))
                elif '2022' in config['dataset_root_valid']:
