...
class Feature_StftPlusIV(nn.Sequential):
    def __init__(self, nfft=512):  ###  frame： 2.55 x 24000 = 61200  （61200/240）+1 = 256
...
class Feature_StftPlusIV_Base(nn.Sequential):
    def __init__(self, nfft=512):  ###  frame： 2.55 x 24000 = 61200  （61200/240）+1 = 256
        super(Feature_StftPlusIV_Base, self).__init__()
        self.stft = torchaudio.transforms.Spectrogram(n_fft=nfft,
                                          hop_length=480,
                                          power=None)
        self.eps = 1e-10
...
#### 符合官方设置的参数
class Feature_MelPlusIV_Base(nn.Sequential):
    """ Mel Spectoram with interchannel phase difference features"""
    def __init__(self, normalize_specs=True, n_mels=64, nfft=1024):
        super().__init__()
        self.stft = torchaudio.transforms.Spectrogram(n_fft=nfft,
                                          hop_length=240,
                                          power=None)
