...
def _read_time_array(fname: str, directory_root: str) -> List:
    ...
    this_time_array = pd.read_csv(fpath_csv, usecols=range(0,5),header=None).values #
...
def load_output_format_file(fname: str, directory_root: str):
    ...
    for _line in _fid:
        ...
        elif len(_words) == 6: # cartesian coordinates [frame number (int)], [active class index (int)], [source number index (int)], [azimuth (int)], [elevation (int)], [distance (int)]
...
class DCASE_SELD_Dataset(Dataset):
    ...
    """
    def __init__(self,
                 directory_root: str = './data/',  # ['/home/yupeng/datasets/dcase2022']
                 list_dataset: str = 'dcase2021t3_foa_overfit_vrgpu.txt', # ['dcase2022_devtrain_all.txt']
                 trim_wavs: float = -1,  # in seconds  -1
                 chunk_size: int = 48000,  # in samples  24000 * dataset_chunk_size_seconds (2.55)
                 chunk_mode: str = 'fixed',  # random
                 return_fname: bool = False,
                 multi_track: bool = False,
                 num_classes: int = 13,
                 ignore_labels: bool = False,
                 labels_backend: str = 'sony',
                 pad_labels: bool = True):  # false
        super().__init__()
        ...
        self._fnames = _read_fnames(directory_root=self.directory_root, list_dataset=self.list_dataset) # 获取 list文件中的 所有 音频的 路径
        for fname in self._fnames:
            audio, fs, duration = _read_audio(fname=fname, directory_root=self.directory_root,
                                              resampler=self.resampler, trim_seconds=self.trim_wavs)   ## audio :整个原音频的数据 fs：采样率 duration：单位 秒
            if not self.ignore_labels:
                if self.labels_backend == 'sony':
                    time_array = _read_time_array(fname=fname, directory_root=self.directory_root)  # 读 csv label
                    ...
                    if self.multi_track:
                        time_array = get_adpit_labels_for_file(_desc_file=time_array, _nb_label_frames=math.ceil(duration * 10),
                                                               num_classes=self.num_classes)
                    else:
                        time_array = get_labels_for_file(_desc_file=time_array, _nb_label_frames=math.ceil(duration * 10), num_classes=num_classes)
                self._time_array_dict[fname] = time_array  # 存放audio标签的字典 {'audio_name':label}
            self._audios[fname] = audio # 存放 audio的数据dict {'aduio_name':data}
            self._fs[fname] = fs #存放 audio的 采样频率
            self.durations[fname] = duration #存放每个音频的 时间 秒 为单位
    ...
    def __getitem__(self, item):
        ...
        elif self.chunk_mode == 'random':  # 这个
        ...
        if not self.ignore_labels:
            if self.labels_backend == 'sony':  # 这个
                ...
                if not self.multi_track:
                    ...
                    start_frame = int(start_sec) * 10
                    end_frame = start_frame + math.ceil(labels_duration / fs * 10)
                    label = time_array[start_frame: end_frame, :]
                    mask = label[:, :13]
                    mask = np.tile(mask, 3)
                    label = mask * label[:, 13:]
                    label = label.transpose(1, 0)  # (39,50)
                    label = label.reshape(3, self.num_classes, -1)  # (3,13,50)
                    #raise NotImplementedError
                else:
                    # TODO Hardcoded fs for laels at 100 ms
                    start_frame = int(start_sec) * 10
                    end_frame = start_frame + math.ceil(labels_duration / fs * 10) + 1  ### 原版本
                    # end_frame = start_frame + math.ceil(labels_duration / fs * 10)
                    ## 原版本
                    ...
                    # if end_frame > time_array.shape[0]:
                    #     time_array = np.concatenate([time_array, np.zeros([end_frame - start_frame - time_array.shape[0], *time_array.shape[1:]])], axis=0)
                    #
                    # label = time_array[start_frame: end_frame, ...]
        else:
            label = np.empty(1)
        ...
        if self.return_fname:
            ...
            return audio, torch.from_numpy(label.astype(np.float32)) # 这个
