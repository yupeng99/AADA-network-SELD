...
class MSELoss_ADPIT(object):
    ...
    def __call__(self, output, target):
        ...
        loss_2 = self._each_calc(output, target_B0B1B0 + pad4B)  # 重叠两个的同类事件和重叠三个的没进行排列。
...
class CausConv2d(nn.Module):
    ...
    def forward(self, x):
        x = nn.functional.pad(x, (1, 1, self.pad, 0), 'constant', 0.0)  # (左右上下)
        x = F.relu(self.bn(self.conv(x)))
        return x
...
class SeldModel(torch.nn.Module):
    def __init__(self, params, out_shape = (8,50,13*3*3)):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.params = params
        self.t = 0
        self.conv_block_list = nn.ModuleList()
        in_feat_shape = (params.batch_size,*params.input_shape)
        out_shape = (params.batch_size,*params.output_shape)
        ...
        if params.dataset_multi_track:
            self.fnn_list.append(
                nn.Linear(params['fnn_size'] if params['nb_fnn_layers'] else self.params['rnn_size'], out_shape[-1],
                          bias=True))
        else:
            self.fnn_list.append(
                nn.Linear(params['fnn_size'] if params['nb_fnn_layers'] else self.params['rnn_size'], 3*13,
                          bias=True))
    ...
    def f(self,x):
        ...
        x = x.transpose(1, 2).contiguous()  # contiguous 相当于深度拷贝
    ...
    def g(self,x):
        ...
        x = self.fnn_list[-1](x)
        x = torch.tanh(x)
        # print(f"x.shape:{x.shape}")  #(16,50,39)
        if self.params.dataset_multi_track == False:
            # x =x.view(-1, 3, self.nb_classes, int(self.t/5))
            x = x.transpose(1,2).contiguous()
            # print(f"x.shape:{x.shape}")
            x = x.view(x.shape[0],3,self.nb_classes,-1).contiguous()
        return x
...
class SeldModel_after(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params):
        ...
        if len(params['f_pool_size']):
            for conv_cnt in range(len(params['f_pool_size'])):
                ...
                self.conv_block_list.append(
                    nn.MaxPool2d((1, params['f_pool_size'][conv_cnt])))
                self.conv_block_list.append(nn.Dropout2d(p=params['dropout_rate']))
...
class SeldModel_online(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.params = params
        ...
        if len(params['f_pool_size']):
            for conv_cnt in range(len(params['f_pool_size'])):
                self.conv_block_list.append(
                    CausConv2d(in_channels=params['nb_cnn2d_filt'] if conv_cnt else in_feat_shape[1],
                               out_channels=params['nb_cnn2d_filt']))
                self.conv_block_list.append(
                    nn.MaxPool2d((params['t_pool_size'][conv_cnt], params['f_pool_size'][conv_cnt])))
                self.conv_block_list.append(nn.Dropout2d(p=params['dropout_rate']))
        ...
        self.gru_input_dim = params['nb_cnn2d_filt'] * int(np.floor(in_feat_shape[-1] / np.prod(params['f_pool_size'])))
        self.gru = torch.nn.GRU(input_size=self.gru_input_dim, hidden_size=params['rnn_size'] * 2,
                                num_layers=params['nb_rnn_layers'], batch_first=True,
                                dropout=params['dropout_rate'], bidirectional=False)
...
class SeldModel_online_after(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.params = params
...
class SeldModel_6conv(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.params = params
            ...
            for conv_cnt in range(len(params['f_pool_size'])):
                self.conv_block_list.append(
                    ConvBlock(in_channels=params['nb_cnn2d_filt'] if conv_cnt else in_feat_shape[1],
                              out_channels=params['nb_cnn2d_filt']))
                self.conv_block_list.append(
                    ConvBlock(in_channels=params['nb_cnn2d_filt'],
                              out_channels=params['nb_cnn2d_filt']))
                self.conv_block_list.append(
                    nn.MaxPool2d((params['t_pool_size'][conv_cnt], params['f_pool_size'][conv_cnt])))
                self.conv_block_list.append(nn.Dropout2d(p=params['dropout_rate']))
