...
class MDAAug(nn.Module):
    ...
    def get_aug_valid_audios(self, audios, magnitudes,target,weights):
        ...
        for i, audio in enumerate(audios):
            ...
            if weights[i][0] != 0: # rotations_aug
            ...
            for k, ops_name in enumerate(self.ops_names[1:]):
                audio = apply_augment(audio, ops_name, magnitudes[i][k+1])
                audio = weights[i][k+1] * audio
            audio = stop_gradient(audio.cuda(), magnitudes[i][k+1])

            if do_reshape:
                audio = audio.squeeze(0)
            trans_audio_list.append(audio)  # 每一次对 原始图像 使用DA操作后 添加到list
    ...
    def explore(self, audios,target):
        ...
        # ba_audios = a_audios.reshape(len(audios), self.n_ops-3, -1)  # （数据个数，DA操作个数，每个DA操作后的数据）
        # print(f"weights.shape:{weights.shape}\tba_audios:{ba_audios.shape}") # ba_audios：[32,10,244800]  244800 = 4 * 61200

        # mixed_audios = [w.matmul(feat) for w, feat in zip(weights[:,3:], ba_audios)]  # 每个对原始图像进行DA后的数据 * weight 累计和
        # mixed_audios = torch.stack(a_audios, dim=0)  # 形成一个新的张量  (64,3072)  (batch,data)
        mixed_audios = a_audios.reshape(len(audios), 4, -1)  # (数据个数,通道数，12000)
