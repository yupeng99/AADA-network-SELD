...
class CRNN10(nn.Module):
    def __init__(self, class_num, in_channels, out_channels=1, pool_type='avg',
        ...
        self.t = 0 # 记录要输出的时长
        self.b = 0 # batch_size
        self.t1 = 0 #记录 f 和 g 改变形状的时长
        self.dim = 0 # 用于Project的输入
    ...
    def f(self,x):
        ...
        # x = self.conv_block2(x, self.pool_type, pool_size=self.pool_size)
        # x = self.conv_block3(x, self.pool_type, pool_size=self.pool_size)
        # x = self.conv_block4(x, self.pool_type, pool_size=(5,2))
        '''(batch_size, feature_maps, time_steps, mel_bins)'''
    ...
    def g(self,x):
        ...
        event_output = event_output.transpose(1, 2)
        if self.multi_track:
            event_output = event_output.view(-1, 3, self.out_channels, self.class_num, self.t)

            b, ch, tra, cls, ts = event_output.shape
            event_output = event_output.permute([0, 4, 1, 2, 3])
            event_output = event_output.view([-1, ts, ch * tra * cls])
        else:
            event_output = event_output.view(-1, self.out_channels, self.class_num, self.t)
            # event_output = event_output.view(-1, self.out_channels, self.class_num, int(self.t / 5))
        return event_output

    def forward(self, x):
        x = self.f(x)
        # print(f"x.shape:{x.shape}")  #torch.Size([32, 16, 512])
        event_output = self.g(x)
        return event_output
