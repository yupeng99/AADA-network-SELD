...
class MDAAug(nn.Module):
    ...
    def predict_aug_params(self, data, mode):
        ...
        # 将无穷小和小于0的数设置为0.001
        processed_input[(weights == float('-inf')) | (weights < 0)|torch.isnan(weights)] = 0.0
    ...
    def get_aug_valid_audios(self, audios, magnitudes):
        ...
        for i, audio in enumerate(audios):
            ...
            for k, ops_name in enumerate(self.ops_names):
                trans_audio = apply_augment(audio, ops_name, magnitudes[i][k])
                # trans_image = self.after_transforms(trans_image)
                trans_audio = stop_gradient(trans_audio.cuda(), magnitudes[i][k])
                if do_reshape:
                    trans_audio = trans_audio.squeeze(0)
                trans_audio_list.append(trans_audio)  # 每一次对 原始图像 使用DA操作后 添加到list
        return torch.stack(trans_audio_list, dim=0)

    def explore(self, audios):
        ...
        a_audios = self.get_aug_valid_audios(audios, magnitudes)  # 增强后的图像数据
        ba_audios = a_audios.reshape(len(audios), self.n_ops, -1)  # （数据个数，DA操作个数，每个DA操作后的数据）
        # print(f"weights.shape:{weights.shape}\tba_audios:{ba_audios.shape}") # ba_audios：[32,10,244800]  244800 = 4 * 61200
        mixed_audios = [w.matmul(feat) for w, feat in zip(weights, ba_audios)]  # 每个对原始图像进行DA后的数据 * weight 累计和
        mixed_audios = torch.stack(mixed_audios, dim=0)  # 形成一个新的张量  (64,3072)  (batch,data)
        mixed_audios = mixed_audios.reshape(len(audios), 4, -1)  # (数据个数,通道数，32，32)
    ...
    def get_training_aug_images(self, audios, magnitudes, weights):

        # visualization

        # if self.iters > 0 and self.iters % 100000 == 0:
        #     self.k_ops += 1
        #     print(f"使用的数据增强个数:{self.k_ops}")
        #
        # if self.iters % self.config["num_iters"] == 0:
        #     self.k_ops = self.config.k_ops
        ...
        if self.k_ops > 0:
            ...
            if self.sampling == 'prob':
                idx_matrix = torch.multinomial(weights,
                                               self.k_ops)  # shape(5,2)无放回采样 ，元素权重是0时，在其他元素被取完之前是不会被取到的。值越大被抽中的概率值的索引越大。 比如 idx_matrix：tensor([[15,  8],[ 5, 11], [13,  7],[14,  0],[ 9,  1]], device='cuda:0')
            elif self.sampling == 'max':
                idx_matrix = torch.topk(weights, self.k_ops, dim=1)[1]  # 选择最大的k_ops个索引
            ...
            for i, audio in enumerate(audios):  # images shape(5,3,32,32)
                ...
                for idx in idx_matrix[i]:
                    ...
                    if (self.iters % self.search_freq == 0):
                        self.fs.write(
                            "第" + str(i) + "个音频" + "augment_method:" + self.ops_names[idx] + "p:" + str(
                                m_pi) + "\n")
                if (self.iters % self.search_freq == 0):
                    self.fs.write("\n")
            ...
            trans_audios = []
            for i, audio in enumerate(audios):
                # pil_image = transforms.ToPILImage()(audio)
        ...
        return feature_aug_audios

    def exploit(self, audios):
        ...
        features_aug_audio = self.get_training_aug_images(audios, magnitudes, weights)
        # if self.features_transform is not None:
        #     features_aug_audio = self.features_transform(aug_audios)
        return features_aug_audio

    def forward(self, audios, mode):
        if mode == 'explore':
            #  return a set of mixed augmented features
            return self.explore(audios)
        elif mode == 'exploit':
            #  return a set of augmented images
            return self.exploit(audios)
        elif mode == 'inference':
            return audios
