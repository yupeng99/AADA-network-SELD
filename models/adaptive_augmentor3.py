...
class MDAAug(nn.Module):
    ...
    def get_aug_valid_audios(self, audios, magnitudes,target,weights):
        ...
        for i, audio in enumerate(audios):
            ...
            if weights[i][1] != 0:
            # trans_audio, target_i = rotations_aug(self.config, audio, target_i, magnitudes[i][0])
                apply_augment(audio, "rotation_noise_aug", magnitudes[i][1])
                audio = weights[i][1] * audio

            if weights[i][2] != 0:
            # trans_audio, target_i = rotations_aug(self.config, audio, target_i, magnitudes[i][0])
                apply_augment(audio, "augmentation_transform_spatial_aug", magnitudes[i][2])
                audio = weights[i][2] * audio


            for k, ops_name in enumerate(self.ops_names[3:]):
